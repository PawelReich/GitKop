...
\begin{itemize}
    \item przenikalność magnetyczna $\mu$
    \item przewodność właściwa $\sigma$
\end{itemize}
...
\end{equation}
gdzie $\sigma$ oznacza konduktywność (przewodność właściwa) materiału, $\mathbf{E}$ oznacza pole elektryczne.
...
Równanie to wskazuje, że interakcja pola z metalem nie jest natychmiastowa, lecz podlega procesom relaksacyjnym zależnym od właściwości materiałowych obiektu oraz częstotliwości fali pobudzającej. Prowadzi to do zjawiska naskórkowości (ang. skin effect), które ogranicza penetrację prądów wirowych do warstw powierzchniowych przy wyższych częstotliwościach, co ma fundamentalne znaczenie dla identyfikacji obiektów \cite{moreland2024inside}.

\subsection{Zjawisko naskórkowości}

Gdy w przewodniku płynie prąd o dużej częstotliwości ma on tendencję do poruszania się po jego powierzchni zewnętrzenej. Jest to zjawisko naskórkowości, wyraźny przy większych częstotliwościach.
Ma ono wpływ na głębokość penetracji fal elektromagnetycznych w głąb badanego obiektu. Zjawisko naskórkowości zaczyna być zauważalne przy częstotliwościach powyżej 100kHz. Jeżeli fala elektromagnetyczna generowana przez wykrywacz metalu sięga badany obiekt tylko na niewielką głębkokość, może zostać błędnie zinterpretowany \cite{moreland2024inside}.

Wzór na głębokość naskórkowości definiuje głębokość, w której amplituda fali maleje do $e^{-1} \approx 36,9\% $ \cite{schmitt2002electromagnetics}: 

\begin{equation}
    \delta = \frac{1}{\sqrt{\pi f \mu \sigma}}
    \label{eq:skin_effect}
\end{equation}
gdzie $f$ to częstotliwość fali elektromagnetycznej, $\mu$ to przenikalność magnetyczna przewodnika, $\sigma$ to konduktywność przewodnika.

\begin{figure}[H]
    \begin{center}
        \includesvg[width=0.4\textwidth]{./img/Skin_depth.svg}
    \end{center}
    \caption{Rozkład przepływu prądu w przewodniku cylindrycznym, pokazany na przekroju poprzecznym.}
    \label{fig:skin_effect}
    \hfill\small źródło: wikipedia.org \cite{skin_effect_svg} 
\end{figure}
